...
const POSTS = [
	...
	{
		// threadTitle 欠落 (IPC のフィールド名不一致等) を混ぜても、
		// currentThreadTitle が throw してスレタイ横のレス数表示まで
		// 巻き添えで固着しないことの回帰データ。
		number: 4,
		name: '名無し',
		mail: '',
		...
		id: '',
		body: '4 レス目',
		threadTitle: undefined as unknown as string,
	},
];
...
vi.mock('$lib/api', () => {
	...
	return {
		...
		fetchThread: vi.fn(async () => [POSTS, { lastModified: null, lastByte: 0, lastCount: 4 }]),
...
test('スレッド読み込み後にレス数 (.t-count-main) と一覧が反映される', async () => {
	...
	// レス一覧が 3 件描画されるまで待つ。
	await vi.waitFor(() => expect(container.querySelectorAll('.post').length).toBe(4));
