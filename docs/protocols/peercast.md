...
PeerCast 本体は、PSTPlayer と **同じマシン** にあるとは限らない。典型的な構成:

| パターン                                | ユースケース                                          |
| --------------------------------------- | ----------------------------------------------------- |
| A. 同一マシン (`localhost`)            | 普通のデスクトップ視聴。最も一般的                    |
| B. LAN 内の別マシン (`192.168.x.x` 等) | ホームサーバ / NAS / 余り PC に PeerCast を常駐させ、視聴 PC からアクセス |
| C. リモートホスト (WAN 越し)            | VPN 経由でアクセス。MVP では対象外、将来検討          |

PSTPlayer は **A, B を MVP から対応**。設定で接続先を切り替え可能。

### 接続パラメータ

- 既定リスニングポート: **7144** (TCP)。ただし PeerCast 本体の設定で変更可
- 既定ホスト: `localhost`。設定 UI で変更可
- 認証:
  - ローカル接続 (A) は通常認証なし
  - リモート / LAN 別マシン (B, C) は PeerCastStation 側の設定次第で HTTP Basic 認証あり

### 接続先の決定優先順位

複数の経路で接続先が示される。**優先順位** (上が強い):

1. **CLI 引数の URL に含まれるホスト/ポート** (例: 外部 YP ブラウザから渡された URL)
2. **設定 UI で指定された PeerCast ホスト/ポート** (恒久的設定)
3. **デフォルト** (`localhost:7144`)

CLI で来たホスト/ポートは **そのセッション限定** で使用し、設定ファイルには上書き保存しない (起動側の渡してきた値を尊重するが、永続化はしない)。

### 接続テスト

設定 UI に「接続テスト」ボタンを設け、現在の設定 (ホスト/ポート/認証) で `getVersionInfo` を呼んで応答を確認できる。LAN 内別マシン構成のセットアップを楽にするため。
