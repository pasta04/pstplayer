...
- **外部ツール登録 (オプション → ツール タブ + 右クリック → ツール)**:
  PeCaRecorder は任意の外部実行ファイルとその引数 (`$x` / `$0` / `$3` 等
  の PeerCast URL 変数置換つき) を登録して右クリックから起動できる仕組み
  を持つ。pstplayer は組み込み libmpv + `tauri-plugin-opener` (= OS デフォ
  ルトブラウザ / アプリで URL を開く) で代替し、ユーザ任意の外部
  プログラム登録は **採用しない**。
  - 「特定のプレイヤーで再生したい」「BBS を専用ビューアで開きたい」等
    の要望は: 「コンタクト URL をコピー」→ ユーザが自分のお好みアプリで
    手動で開く運用にする
- **チャット URL を開く**: 初代 Yellow Page に実装されていた「YP 上で
  チャットができる」機能のためのフィールド。現行 YP では機能停止されて
  いるため非対応
