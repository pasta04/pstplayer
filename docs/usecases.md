...
- アクター: U4 (Desktop)
- トリガ: ハブから複数チャンネルを順次クリック
- 主シナリオ:
  1. ハブで YP / お気に入り / 履歴からチャンネル A をクリック
     → ビューア A が開く
  2. ハブはそのまま操作可能なので、続けて B をクリック → ビューア B
  3. 同様に N 個 (リソース上限まで) 並べる
  4. 各ビューアは独立した BBS / 書き込み欄を持つ
- 期待: OS のウィンドウ管理 (タイル / マルチモニタ) を活かす
- 制約: CPU / メモリは「libmpv × N」。常識的には 2-4 本まで
- 注意: 同じチャンネルの 2 重起動は禁止 (フォーカスを移す)
...
### UC-08: 自動配信録画 (Desktop ホスト)

- アクター: U1 / U3
- トリガ: Desktop 側で常時動いている何らかのプロセスがあること
- 期待: pst-server を別途立てなくても、Desktop 利用者の PC 上で
  「お気に入り配信を見逃さない」を実現する
- **配信録画タスクをどこで動かすか** は 3 案あり、いずれもユーザーから
  見た「ウィンドウが増える / 増えない」が違うので方針を選ぶ:

#### 案A: トレイ常駐 (ウィンドウなし)

- PSTPlayer.exe 起動時はウィンドウを出さずタスクトレイに常駐
- トレイメニューから「視聴ウィンドウを開く」「YP」「設定」
- 自動録画は裏のタスクで走る
- PC を起動している時間しか動かない
- Tauri の tray API を使用 (OS 別の挙動差は要検証)

#### 案B: 視聴ウィンドウ起動中だけ動く (現状維持 + 隠れタスク)

- PSTPlayer を普通に起動 (1 ウィンドウ視聴) → アプリ内タスクが裏で
  AutoRecorder を回す
- ウィンドウを閉じると録画も止まる
- 一番シンプルだが「録画したいから常に起動しっぱなし」が要る

#### 案C: 自動録画は pst-server に任せる (分業)

- Desktop アプリは視聴専用 (今と同じ)
- 自動録画したいユーザーは pst-server を別途常駐させる:
  - Pi / NAS 等のサーバ
  - Windows サービスとして PC にインストール
  - Linux systemd ユーザー unit
- 「録画専用 daemon を立てる」運用は責務が明確で堅牢
- ライト用途には重い

> どの案を採るかは ADR-0006 と合わせて要決定。複数視聴 (UC-03) の
> ウィンドウ構成とは独立に選べる。
