...
### UC-08: 自動配信録画 (Desktop ハブ)

- アクター: U1 / U3
- トリガ: ハブが常時起動中、お気に入りに `auto_record = true` の
  ルールあり
- 主シナリオ: UC-07 とロジックは同じ。Desktop ハブが PeerCastStation
  を polling する
- 期待: pst-server を立てなくても自動録画したい人向け
- 制約: PC を起動している時間しか動かない (常時起動には向かないが、
  「いつもアプリを立てっぱなし」運用なら成立)
- 注意: ハブとビューアで重複録画にならないよう、AutoRecorder と手動
  録画の channel_id 重複は 409 で防ぐ
