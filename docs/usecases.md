...
- アクター: U4 (Desktop)
- 採用方針 (ADR-0006): **`pstplayer` を複数プロセス起動 (= ウィンドウ
  複数)**
- 主シナリオ:
  1. `pstplayer` で配信 A を視聴中
  2. 右クリック → 「別ウィンドウで開く」 / お気に入りや YP (pst-server
     Web) のリンクから別チャンネルを起動 → 子プロセスで配信 B
  3. 同様に N 個。各々独立した PlayerEngine + BBS
  4. 同じ channel_id を 2 度開いた場合は既存ウィンドウにフォーカス
     (single_instance)
- 期待: OS 標準のウィンドウ管理 (タイル / マルチモニタ) を活かす
- 制約: CPU / メモリは「libmpv × N」。常識的には 2-4 本まで
- 1 ウィンドウ内タイル表示は採用しない (ADR-0006 §A の理由を参照)
