...
- [x] ショートカット「切断してウィンドウを閉じる」(`Alt+X` / PCRPlayer 互換) を追加
      (設定 → ショートカット タブで再割り当て可)
- [ ] **GUI でのショートカット再割り当ての拡充** (将来): 対象 action の追加・
      プロファイル切替・より発見しやすい UX。基盤 (設定 → ショートカット タブ:
      フェーズ 2.4) は実装済なので、対象拡大と UX 改善が中心 — **優先度: 中**
