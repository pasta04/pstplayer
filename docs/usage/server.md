...
ヘッダ右上の **⊞ グリッド** ボタンで、画面が `<video>` × N のタイル
レイアウトに切り替わります。1 端末で複数チャンネルを「ながら見」する
ためのモードです。

- 切替後、下のチャンネル一覧の行をタップでタイルに追加 (同 ch は重複
  不可、既に追加済みなら focus へ)
- タイル本体タップ → そのタイルだけ unmute、他は mute (iOS Safari の
  `<video>` 同時再生制約に合わせるため。サウンドは常に 1 本)
- 各タイル右上の **⏺** で個別録画 / 停止、右下の **✕** で個別削除
- お気に入りルールにマッチするタイルは **★** マーク + favorites.color
  の枠線
- レスポンシブ列数: モバイル 2 / タブレット 3 / PC 4 / ワイドモニタ 5

CPU と帯域の都合上、実用上限の目安は **スマホ 2〜4 本 / タブレット
4〜6 本 / PC 6〜9 本** です。それ以上は再生が間に合わずカクつきます。
タイルの **✕** で順次減らしてください。
